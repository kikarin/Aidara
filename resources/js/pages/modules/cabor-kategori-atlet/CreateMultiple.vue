    ...
    try {
        await router.post(
            `/cabor-kategori/${props.caborKategori.id}/atlet/store-multiple`,
            {
                ...
                posisi_atlet: posisiAtlet.value,
            },
            {
                onSuccess: () => {
...
<template>
    <PageCreate title="Tambah Multiple Atlet" :breadcrumbs="breadcrumbs" back-url="/cabor-kategori" :use-grid="true">
        <div class="space-y-6">
            ...
            <!-- Input Posisi Atlet (opsional) -->
            <div class="flex items-center gap-4">
                <div class="w-64">
                    <label class="mb-2 block text-sm font-medium">Posisi Atlet (Opsional)</label>
                    <Input v-model="posisiAtlet" placeholder="Masukkan posisi atlet" />
                </div>
            </div>
